			...
			for move in self:
				...
				if uom:
					move.secondary_done_qty = sum(
						self.env['uom.uom'].sudo().browse(line_uom_id)._compute_quantity(qty, uom, round=False)
		...
		for move in self:
			...
			if not move_lines:
				if quantity_done:
					...
					move_line = self.env['stock.move.line'].sudo().create(dict(move._prepare_move_line_vals(), qty_done=quantity_done))
...
class StockMoveLine(models.Model):
	...
	@api.depends('product_id','product_id.uom_id','reserved_uom_qty','qty_done','product_id.secondary_uom_id')
	def _compute_secondary_qty(self):
		for move_line in self:
			if move_line.product_id.secondary_uom:
				...
				uom_quantity = move_line.product_id.uom_id._compute_quantity(move_line.reserved_uom_qty or move_line.qty_done, move_line.product_id.secondary_uom_id, rounding_method='HALF-UP')
